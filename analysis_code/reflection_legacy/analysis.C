 ...
 */
void analysis() {
    const char* input_file = "with_lead/output.root";
    //const char* input_file = "~/Desktop/production_output_with_lead_Cs137.root";
    //const char* input_file = "~/Desktop/production_output_with_lead_Co57.root";
    //const char* input_file = "/home/sgwon/MD/data/reflection_test/Zn65/production_output_with_lead_Zn65.root";
    ...
    TH1D* hist_npe[2];
    for (int i = 0; i < 2; ++i) {
        hist_npe[i] = new TH1D(Form("h_npe_ch%d", i), Form("NPE Ch%d (Gain=%.1E);NPE;Counts", i, gain[i]), 500, 0, 50);
    }

    TH1D* hist_total = new TH1D("Total NPE", "Total NPE;NPE;Events", 1000, 0, 1000);
