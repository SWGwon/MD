...
void fit_led_only(int ch = 0, const char *ledPattern = "output_charge_run2_%dV.root") {
    ...
    for (int v : voltages) {
        ...
        double max_bin_center = h->GetBinCenter(h->GetMaximumBin());
        h->GetXaxis()->SetRange(0, 0); 

        TF1 *f_ped_pre = new TF1("f_ped_pre", "gaus", max_bin_center - 0.1, max_bin_center + 0.1);
        h->Fit(f_ped_pre, "RQN");
        ...
        // --- 3. SPE 메인 피팅 ---
        TF1 *f_spe = new TF1("f_spe", spe_model_led, -0.5, q0 + q1_guess * 5.0, 7);
        ...
        f_spe->SetParLimits(2, q0-0.15, q0+0.15); 
        f_spe->SetParLimits(3, 0.01, 0.2); 
        f_spe->SetParLimits(4, q1_guess*0.7, q1_guess*1.3); 
        f_spe->SetParLimits(5, q1_guess*0.1, q1_guess*1.0); 
    ...
    TF1 *f_gain_fit = new TF1("f_gain_fit", "[0]*TMath::Power(x, [1])", 1500, 2300);
    f_gain_fit->SetParameters(1e-25, 10.0);
    gr_gain->Fit(f_gain_fit, "R");
    
    cg->SaveAs(TString::Format("gain_curve_led_only_ch%d.png", ch));
}
